...
void printPar();
std::vector<int> GenerateQTLData(std::vector<boost::shared_ptr<short[]> > haps, int snpnum, int ploidy,int samplenum, std::string prefix);
std::vector<int> GenerateBinaryData(std::vector<boost::shared_ptr<short[]> > haps, int snpnum, int casenum, int ctrlnum,int ploidy, std::string prefix);
...
int main(int argc, char* argv[]) {
  ...
  printPar();
  rng.seed(par.seed);
  std::vector<boost::shared_ptr<short[]> > haplotypes=GenerateHaplotype(par.allele,par.hap,par.snp);
  std::vector<int> hapcount;
  if(par.qtlnum == 0){
	 hapcount=GenerateBinaryData(haplotypes,par.snp,par.casenum,par.ctrlnum, par.ploidy,par.output);
  }else{
	  hapcount=GenerateQTLData(haplotypes,par.snp,par.ploidy,par.qtlnum,par.output);
  }

...
template
 bool next_combination(std::vector<int>::iterator n_begin, std::vector<int>::iterator  n_end,
...
void HaplotypeEM::generateAllPossibleHap() {
  ...
  std::vector<short> alleleType1 =
      getAlleleType(this->InterMediate, this->UniqueGenotypeIdx, 0,this->data->getNumOfChrSet());
  std::vector<short> alleleType2 =
      getAlleleType(this->InterMediate, this->UniqueGenotypeIdx, 1,this->data->getNumOfChrSet());
...
bool ExistsHap(boost::shared_ptr<short[]> hap,std::vector< boost::shared_ptr<short[]> > &v ){
	for(int i=0;i<v.size();i++){
		if(hap[0]==v[i][0]&&hap[1]==v[i][1])
			return true;
	}
	return false;
}
...
long choose(boost::shared_ptr<int[]> got, int n_chosen, int len, int at, int max_types, std::vector< std::vector<int> >& combination)
{
        int i;
        long count = 0;
        if (n_chosen == len) {
...
//				for(int whichhc=0;whichhc<hc.hapIdx.size();whichhc++){
...
//	 std::cout<<"Haplotype:"<< OneGenotypeExpandedHaplo::haploType[i][0]<< OneGenotypeExpandedHaplo::haploType[i][1]<<",freq:"
...
//    	std::cout<<"\n";
    	for(int p=0;p<this->data->getNumOfChrSet();p++){
    		short CurHapAllele1=OneGenotypeExpandedHaplo::haploType[CurCombination.hapIdx[p]][0];
    		short CurHapAllele2=OneGenotypeExpandedHaplo::haploType[CurCombination.hapIdx[p]][1];
    		int whichPloidy=locate(this->InterMediate,samples[sampleIdx],this->data->getNumOfChrSet(),CurHapAllele1,isPhased);
//    		std::cout<<"a1:"<<CurHapAllele1<<",a2:"<<CurHapAllele2<<",p:"<<whichPloidy<<"\n";
    		BOOST_ASSERT(whichPloidy!=-1);
    		this->PhasedData[samples[sampleIdx]][this->phased][whichPloidy]=CurHapAllele2;
    	}

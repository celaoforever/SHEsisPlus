...
boost::shared_ptr<SHEsis::SHEsisData> GenerateRandomData(int sampleNum, int snpNum, int chrSetNum){
	double probabilities[]={0,0.5,0.5};//0.04 missing phenotype for individuals
...
test_main(int,char*[])
{
	...
	int snpNum=1;
	int chrSetNum=10;
	boost::shared_ptr<SHEsis::SHEsisData> testdata=GenerateRandomData(sampleNum,snpNum,chrSetNum);

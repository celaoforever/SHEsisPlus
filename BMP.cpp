 ...
 */
void BMP_point(BMP* bmp, int x, int y, RGB rgb) {
  if (!bmp || x < 0 || y < 0) return;
 ...
 */
void BMP_line(BMP* bmp, int x0, int y0, int x1, int y1, RGB rgb) {
  if ((rgb >> 24) == 0xff) return;
 ...
 */
void BMP_rect(BMP* bmp, int x, int y, int w, int h, RGB rgb) {
 ...
 * Purpose:	Fills a rectangle with a color.
 */
void BMP_fillrect(BMP* bmp, int x, int y, int w, int h, RGB rgb) {
  while (h > 0) {
    BMP_hline(bmp, x, x + w - 1, y, rgb);
    h--;
    y++;
 ...
 */
void BMP_clear(BMP* bmp, RGB rgb) {
 ...
 */
void BMP_hline(BMP* bmp, int x0, int x1, int y, RGB rgb) {
  if (x0 > x1) {
    int tmp = x1;
    x1 = x0;
    x0 = tmp;
 ...
 */
void BMP_vline(BMP* bmp, int x, int y0, int y1, RGB rgb) {
  if (y0 > y1) {
    int tmp = y1;
    y1 = y0;
    y0 = tmp;
 ...
 */
int BMP_draw_string(BMP* bmp, const char* string, int x, int y, RGB color,
                    double radian) {
  int originx = x;
  int originy = y;
  char ch;
  ...
  if (!bmp || !string) return 0;
  if (x >= bmp->width || y >= bmp->height || !*string) return 0;
  //----------
  ...
  while ((ch = *s++)) {
    ...
    if (ch > 'z') continue;
    if (ch > ' ' && ch <= 'z') ix = FONT_HEIGHT * (ch - 33);
      ...
      for (i = 0; i < FONT_HEIGHT; i++) {
        ...
        while ((ch2 = *s2++)) {
          ...
          if (draw) {
            ...
            int newx = (int)round((oldx - originx) * cos(radian) -
                                  (oldy - originy) * sin(radian) + originx);
            int newy = (int)round((oldx - originx) * sin(radian) +
                                  (oldy - originy) * cos(radian) + originy);
 ...
 */
int BMP_string_width(char* string) {
  char ch, *s;
  int width = 0;
 ...
 */
int BMP_draw_mini_string(BMP* bmp, const char* string, int x, int y,
                         RGB color) {
  char ch;
  const char* s;
  unsigned long r, g, b;
...
#define MINI_HEIGHT (8)
  ...
  while ((ch = *s++)) {
    ...
    if (ch > 'z') continue;
    if (ch > ' ' && ch <= 'z') ix = MINI_HEIGHT * (ch - 33);
    ...
    if (ix >= 0) {
      ...
      for (i = 0; i < MINI_HEIGHT; i++) {
        ...
        while ((ch2 = *s2++)) {
          ...
          if (draw) BMP_point(bmp, x + j, y + i, color_to_use);
          j++;
        }
      }
 ...
 */
int BMP_draw_narrow_numbers(BMP* bmp, char* string, int x, int y, RGB color) {
  char ch, *s;
 ...
 */
RGB BMP_getpixel(BMP* bmp, int x, int y) {
  if (!bmp || x < 0 || y < 0) return 0;
 ...
 */
int BMP_write(BMP* bmp, const char* path) {
  FILE* f;
#define HDRLEN (54)

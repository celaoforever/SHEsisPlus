...
static void f3xact(int* nrow, int* irow, int* ncol, int* icol, double* dlp,
                   int* mm, double* fact, int* ico, int* iro, int* it, int* lb,
                   int* nr, int* nt, int* nu, int* itc, int* ist, double* stv,
                   double* alen, const double* tol);
static void f4xact(int* nrow, int* irow, int* ncol, int* icol, double* dsp,
                   double* fact, int* icstk, int* ncstk, int* lstk, int* mstk,
                   int* nstk, int* nrstk, int* irstk, double* ystk,
                   const double* tol);
static void f5xact(double* pastp, const double* tol, int* kval, int* key,
                   int* ldkey, int* ipoin, double* stp, int* ldstp, int* ifrq,
                   int* npoin, int* nr, int* nl, int* ifreq, int* itop,
                   int* ipsh);
static void f6xact(int* nrow, int* irow, int* iflag, int* kyy, int* key,
...
static void f10act(int* nrow, int* irow, int* ncol, int* icol, double* val,
...
/*
  ...
  -----------------------------------------------------------------------
*/
void f2xact(int* nrow, int* ncol, double* table, int* ldtabl, double* expect,
            double* percnt, double* emin, double* prt, double* pre,
            double* fact, int* ico, int* iro, int* kyy, int* idif, int* irn,
...
/* TOL is chosen as the square root of the smallest relative spacing. */
  ...
  static int kval, kmax, jkey, last, ipsh, itmp, itop, jstp, ntot, jstp2, jstp3,
      jstp4, i, ii, j, k, n, iflag, ncell, ifreq, chisq, ikkey, ikstp, ikstp2,
      k1, kb, kd, ks, i31, i32, i33, i34, i35, i36, i37, i38, i39, i41, i42,
      i43, i44, i45, i46, i47, i48, i310, i311, nco, nrb, ipn, ipo, itp, nro,
      nro2;
  static double dspt, dd, df, ddf, drn, dro, emn, obs, obs2, obs3, pastp, pv,
      tmp;
  double d1;
#ifndef USING_R
  double d2;
  ...
  if (*ncol <= 1) prterr(4, "NCOL must be at least 2");
  ...
  *pre = 0.;
  ...
  for (i = 1; i <= *ncol; ++i) {
    ico[i] = 0;
    for (j = 1; j <= *nrow; ++j) ico[i] += (int)table[j + i * *ldtabl];
  }
  ...
  */
  ...
  L_ERR_5:
    prterr(5,
           "The hash table key cannot be computed because "
           "the largest key\n"
...
Outer_Loop:
  ...
  n = ico[kb];
  ntot = 0;
  for (i = kb + 1; i <= nco; ++i) ntot += ico[i];

L150:
  /* Arc to daughter length=ICO(KB) */
  for (i = 1; i <= nro; ++i) irn[i] = iro[i] - idif[i];
...
L240:
  ...
  if (k1 > 1) {
    ...
    if (dlp[itp] > 0.) {
      ...
      dlp[itp] = 0.;
      f3xact(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dlp[itp], &ntot, fact,
             &iwk[i31], &iwk[i32], &iwk[i33], &iwk[i34], &iwk[i35], &iwk[i36],
             &iwk[i37], &iwk[i38], &iwk[i39], &rwk[i310], &rwk[i311], &tol);
      dlp[itp] = min(0., dlp[itp]);
      /* Compute shortest path */
      dsp[itp] = dspt;
      f4xact(&nro2, &irn[nrb], &k1, &ico[kb + 1], &dsp[itp], fact, &iwk[i47],
             &iwk[i41], &iwk[i42], &iwk[i43], &iwk[i44], &iwk[i45], &iwk[i46],
             &rwk[i48], &tol);
      ...
      if ((irn[nrb] * ico[kb + 1]) > ntot * emn) {
        ...
        for (i = 0; i < nro2; ++i)
          for (j = 1; j <= k1; ++j)
            if (irn[nrb + i] * ico[kb + j] >= ntot * *expect) ncell++;
  ...
  if (pastp <= obs3) {
    ...
    *pre += (double)ifreq * exp(pastp + drn);
    ...
    if (chisq) {
      df = (double)((nro2 - 1) * (k1 - 1));
#ifdef USING_R
      pv = pgamma(fmax2(0., tmp + (pastp + drn) * 2.) / 2., df / 2.,
                  /*scale = */ 1.,
                  /*lower_tail = */ FALSE,
                  /*log_p = */ FALSE);
#else
      ...
      *pre += (double)ifreq * exp(pastp + drn) * pv;
      ...
      d1 = pastp + ddf;
      f5xact(&d1, &tol, &kval, &key[jkey], ldkey, &ipoin[jkey], &stp[jstp],
             ldstp, &ifrq[jstp], &ifrq[jstp2], &ifrq[jstp3], &ifrq[jstp4],
             &ifreq, &itop, &ipsh);
      ipsh = (0);
    }
  }
...
L310:
  ...
  do {
    ...
    jstp = *ldstp - jstp + 2;
    jstp2 = (*ldstp << 1) + jstp;
    for (i = 1; i <= *ldkey << 1; ++i) key2[i] = -9999;
...
void f3xact(int* nrow, int* irow, int* ncol, int* icol, double* dlp, int* mm,
  ...
  if (irow[*nrow] <= irow[1] + *ncol) {
    f10act(nrow, &irow[1], ncol, &icol[1], &val, &xmin, fact, &lb[1], &nu[1],
           &nr[1]);
  }
  if (!xmin) {
    if (icol[*ncol] <= icol[1] + *nrow) {
      f10act(ncol, &icol[1], nrow, &irow[1], &val, &xmin, fact, &lb[1], &nu[1],
             &nr[1]);
    }
  }
...
LnewNode: /* Setup to generate new node */
  ...
  nct = ico[1];
  lb[1] =
      (int)((double)((nrt + 1) * (nct + 1)) / (double)(nn + nr1 * nc1s + 1) -
            *tol) -
      1;
  nu[1] =
      (int)((double)((nrt + nc1s) * (nct + nr1)) / (double)(nn + nr1 + nc1s)) -
      lb[1] + 1;
  nr[1] = nrt - lb[1];
...
L120:
  ...
  if (lev < nc1s) {
    ...
    nct = ico[lev];
    lb[lev] =
        (int)((double)((nrt + 1) * (nct + 1)) / (double)(nn1 + nr1 * nc1 + 1) -
              *tol);
    nu[lev] =
        (int)((double)((nrt + nc1) * (nct + nr1)) / (double)(nn1 + nr1 + nc1) -
              lb[lev] + 1);
    nr[lev] = nrt - lb[lev];
    goto L120;
  }
  ...
  if (nro == 2) {
    ...
    if (key < 0) error("Bug in FEXACT: gave negative key \n");
...
L200: /* Pop item from stack */
  if (nitc > 0) {
    ...
    /* Set up nt array */
    nt[1] = nn - ico[1];
    for (i = 2; i <= nco; ++i) nt[i] = nt[i - 1] - ico[i];

    /* Test for optimality (L90) */
    xmin = (0);
    if (iro[nro] <= iro[irl] + nco) {
      f10act(&nro, &iro[irl], &nco, &ico[1], &val, &xmin, fact, &lb[1], &nu[1],
             &nr[1]);
    }
    if (!xmin && ico[nco] <= ico[1] + nro)
      f10act(&nco, &ico[1], &nro, &iro[irl], &val, &xmin, fact, &lb[1], &nu[1],
             &nr[1]);
    if (xmin) {
      if (vmn > val) vmn = val;
      goto L200;
...
void f4xact(int* nrow, int* irow, int* ncol, int* icol, double* dsp,
            double* fact, int* icstk, int* ncstk, int* lstk, int* mstk,
            int* nstk, int* nrstk, int* irstk, double* ystk,
            const double* tol) {
  /* System generated locals */
  ...
  L60:
    ...
    if (irt == ict) {
      ...
      f11act(&icstk[istk * *ncol + 1], &j, &nco,
             &icstk[(istk + 1) * *ncol + 1]);
      ikh = irt - ict;
      f8xact(&irstk[istk * *nrow + 1], &ikh, &i, &nro,
             &irstk[(istk + 1) * *nrow + 1]);
    } else {
      --nro;
      f11act(&irstk[istk * *nrow + 1], &i, &nro,
             &irstk[(istk + 1) * *nrow + 1]);
      ikh = ict - irt;
      f8xact(&icstk[istk * *ncol + 1], &ikh, &j, &nco,
             &icstk[(istk + 1) * *ncol + 1]);
    }
...
L100:
  ...
  for (;; ++l) {
    ...
    y = ystk[istk];
    if (n == 1) {
      if (irstk[l + istk * *nrow] < irstk[l - 1 + istk * *nrow]) goto L60;
    } else if (n == 2) {
      if (icstk[l + istk * *ncol] < icstk[l - 1 + istk * *ncol]) goto L60;
    }
  }
}
...
void f5xact(double* pastp, const double* tol, int* kval, int* key, int* ldkey,
            int* ipoin, double* stp, int* ldstp, int* ifrq, int* npoin, int* nr,
            int* nl, int* ifreq, int* itop, int* ipsh) {
  /* Local variables */
      ...
      /* KH
                prterr(7, "LDSTP is too small for this problem.\n"
                       "It is not possible to estimate the value of LDSTP "
                              "required,\n"
                                     "but twice the current value may be
         sufficient.");
      */
      prterr(7,
...
/*
  ...
  -----------------------------------------------------------------------
*/
void f6xact(int* nrow, int* irow, int* iflag, int* kyy, int* key, int* ldkey,
            int* last, int* ipn) {
  int kval, j;
...
void f8xact(int* irow, int* is, int* i1, int* izero, int* knew) {
  int i;
...
void f10act(int* nrow, int* irow, int* ncol, int* icol, double* val, int* xmin,
  ...
  for (i = *nrow - 2; i >= 1; --i) nd[i] += nd[i + 1];
...
/*
  ...
  -----------------------------------------------------------------------
*/
void f11act(int* irow, int* i1, int* i2, int* knew) {
  int i;
...
void prterr(int icode, char* mes) { /*error("FEXACT error "+int2str(icode)); */
...
int iwork(int iwkmax, int* iwkpt, int number, int itype) {
  ...
  if (*iwkpt > iwkmax) prterr(40, "Out of workspace.");
